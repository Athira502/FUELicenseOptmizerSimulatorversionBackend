...
from app.service.chatgpt import call_chatgpt_api
import os
import datetime
import traceback
    ...
    try:
        ...
        for role in distinct_roles:
            ...
            for record in role_records:
                if record.AGR_NAME == role:
                    role_description = record.TTEXT or ""
                    break
...
#
# from app.models.database import engine
# from app.models.dynamic_models import _BaseLiceData, _BaseAuthData, create_lice_data_model, create_auth_data_model
# from app.service.chatgpt import call_chatgpt_api
# import os
# import datetime
# import traceback
...
#          print(f"Error getting dynamic models for client '{client_name}': {e}")
#          logger.error(f"Error getting dynamic models for client '{client_name}': {e}", exc_info=True)
#          return {"error": f"Invalid client name or configuration error for '{client_name}'", "details": str(e), "status_code": 400}
#
#
#     try:
#         query = db.query(_BaseLiceData).filter(
...
#
#     except Exception as e:
#         print(f"Unexpected error during license optimization query/processing for client {client_name} and system id {system_id}: {e}")
#         logger.error(f"Unexpected error during license optimization query/processing for client {client_name} and system id {system_id}: {e}",
#                      exc_info=True)
#         traceback.print_exc()
#         return {"error": f"An unexpected server error occurred during optimization.", "details": str(e), "status_code": 500}
#
#
#
#
